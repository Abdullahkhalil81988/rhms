...
public class HealthAnalytics {
    ...
    public AnalyticsResult analyzePatientData(Patient patient) {
        ...
        if (medHistory != null && !medHistory.getConsultations().isEmpty()) {
        ...
        if (medHistory.getConsultations() != null) {
            for (Feedback feedback : medHistory.getConsultations()) {
                ...
                if (prescription != null) {
                    medications.put(prescription.getMedication(), prescription.getDosage());
                }
            }
        } else {
            // Handle case when no consultations exist
            result.addMessage("No past consultations found in medical history");
            return;
        }
